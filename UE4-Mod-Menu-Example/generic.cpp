...
UE_UObject TUObjectArray::FindObjectPartial(const std::string& name, const std::string& name2) const {
    for (uint32 i = 0u; i < NumElements; i++) {
        UE_UObject object = GetObjectPtr(i);

        std::size_t found = object.GetFullName().find(name);
        std::size_t found2 = object.GetFullName().find(name2);
        if (name2 != "")
        {
            if (object && found != std::string::npos && found2 != std::string::npos)
            {
                return object;
            }
        }
        ...
        {
            if (object && found != std::string::npos)
            {
                return object;
            }
        }
...
UE_UObject TUObjectArray::FindObjectByTypeAndContainsString(const std::string& type, const std::string& persistent, const std::string& objectname) const {
    for (uint32 i = 0u; i < NumElements; i++) {
        UE_UObject object = GetObjectPtr(i);

        std::vector<std::string> type_vector;
        StringFunctionality::split(object.GetFullName(), type_vector, ' ');

        std::size_t found = type_vector[0].find(type);
        std::size_t found2 = type_vector[1].find(persistent);
        std::size_t found3 = type_vector[1].find(objectname);

        type_vector.clear();

        if (persistent != "")
        {
            if (object && found != std::string::npos && found2 != std::string::npos && found3 != std::string::npos)
            {
                return object;
            }
        }
        ...
        {
            if (object && found != std::string::npos && found3 != std::string::npos)
            {
                return object;
            }
        }
