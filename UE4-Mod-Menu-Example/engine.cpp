...
std::unordered_map<std::string, decltype(&engines[0])> games = {
    ...
    {"SessionGame-Win64-Shipping", &engines[0]},
    //{"Four_27_1-Win64-Shipping", &engines[0]},
    {"SCUM", &engines[0]},
...
STATUS EngineInit(std::string game, void* image) {
    ...
    if (GetModuleFileNameEx(NULL, 0, Buffer, MAX_PATH))
    {
        // At this point, buffer contains the full path to the executable
        game_path = *Buffer;

        game_name = remove_extension(base_name(game_path));
    }
    else
    {
